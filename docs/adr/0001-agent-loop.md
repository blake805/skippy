...
Status: accepted (Phase 2), amended by ADR 0006 and ADR 0007

> **Amendments.** The core decision — one continuous tool loop, one model driving
> it — stands unchanged and is why this ADR is still here. Two things in it are
> now wrong:
>
> - The **dual-lane arrangement** described under Consequences is superseded by
>   [ADR 0006](0006-single-runtime-coding-agent.md). There is no shop lane and no
>   `mode` dispatch: `SkippyPipeline` was deleted and archived at tag `shop-v1`.
>   Where this ADR says "the shop lane is *not* migrated", read "the shop lane no
>   longer exists".
> - The `heavy` role is **Qwen3-Coder-480B-A35B-4bit**, not GLM-5.2. See
>   [ADR 0007](0007-model-roles-and-cloud-escalation.md), which also records the
>   benchmark behind the "one model drives the loop" reasoning below: alternating
>   models does destroy prompt-cache locality, and the measured cost is a 59.9s
>   re-prefill against a 2.8s warm step.
