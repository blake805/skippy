...
> **Amendment.** The decision — `_serve_socket` owns the only `receive_text()` on a
> socket, and anything needing a mid-task answer goes through `pending_responses`
> — stands, and the compatibility bridge for clients that do not echo `task_id` is
> live on `main`. What is gone is the caller: the three `SkippyPipeline` approval
> sites named below (`tormach_ssh`, `request_terminal_execution`, and the `DEPLOY`
> gate) were deleted by [ADR 0006](0006-single-runtime-coding-agent.md), along with
> `await_authorization`. The agent runtime's approval gates are the only consumers
> now. Read the shop references as the history that motivated the fix.
